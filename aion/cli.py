...
def main():
    ...
    parser.add_argument(
        "program_file",
        help="Path to AION program JSON file"
    )
    parser.add_argument(
        "--input", "-i",
        help="Path to input data file (CSV, JSON)"
    )
    parser.add_argument(
        "--output", "-o",
        help="Path to output file (CSV, JSON)"
    )
    parser.add_argument(
        "--explain", "-e",
        action="store_true",
    ...
    if isinstance(result.data, pd.DataFrame):
        if result.data.empty:
            ...
            print(f"Result: {len(result.data)} rows, {len(result.data.columns)} columns")
