...
class AIONValidator:
    ...
    def validate_program(self, program: Dict[str, Any]) -> List[ValidationError]:
        ...
        if "pipeline" not in program:
            errors.append(
                ValidationError(
                    path="root", message="Program must contain a 'pipeline' field"
                )
            )
            return errors
        ...
        if not isinstance(pipeline, list):
            errors.append(
                ValidationError(
                    path="pipeline", message="Pipeline must be a list of tasks"
                )
            )
            return errors
    ...
    def _validate_task(self, task: Dict[str, Any], path: str) -> List[ValidationError]:
        ...
        if not isinstance(task, dict):
            errors.append(
                ValidationError(path=path, message="Task must be a dictionary")
            )
            return errors
        ...
        if "task" not in task:
            errors.append(
                ValidationError(
                    path=f"{path}.task", message="Task must have a 'task' field"
                )
            )
            return errors
        ...
        if not isinstance(task_type, str):
            errors.append(
                ValidationError(
                    path=f"{path}.task", message="Task type must be a string"
                )
            )
            return errors
    ...
    def _validate_filter_task(
        self, task: Dict[str, Any], path: str
        ...
        if not isinstance(condition, dict):
            errors.append(
                ValidationError(
                    path=f"{path}.condition", message="Condition must be a dictionary"
                )
            )
            return errors
    ...
    def _validate_sort_task(
        ...
        if not isinstance(operation, dict):
            errors.append(
                ValidationError(
                    path=f"{path}.operation", message="Operation must be a dictionary"
                )
            )
            return errors
    ...
    def _validate_transform_task(
        ...
        if not isinstance(mapping, dict):
            errors.append(
                ValidationError(
                    path=f"{path}.mapping", message="Mapping must be a dictionary"
                )
            )

        return errors

    def _validate_model_call_task(
        self, task: Dict[str, Any], path: str
    ) -> List[ValidationError]:
    ...
    def _validate_aggregate_task(
        ...
        if not isinstance(group_by, list):
            errors.append(
                ValidationError(
                    path=f"{path}.group_by", message="Group by must be a list of fields"
                )
            )
            return errors
    ...
    def _validate_export_task(
        ...
        if not isinstance(file_path, str):
            errors.append(
                ValidationError(
                    path=f"{path}.file_path", message="File path must be a string"
                )
            )
        ...
        if "format" in task:
            ...
            if not isinstance(format_val, str):
                errors.append(
                    ValidationError(
                        path=f"{path}.format", message="Format must be a string"
                    )
                )
